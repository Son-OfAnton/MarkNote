...
class VersionControlManager:
    ...
    def get_version_content(self, note_id: str, version_id: str) -> Tuple[str, Dict[str, Any]]:
        ...
        if not version and len(history) > 0:
            ...
            matching_versions = [
                v for v in history if version_id in v["version_id"]]
    ...
    def compare_versions(self, note_id: str, old_version_id: str,
        ...
        if new_version_id:
            ...
            new_content, _ = self.get_version_content(
                note_id, latest["version_id"])
    ...
    def purge_history(self, note_id: str) -> bool:
        """
        ...
        """
        try:
            # Get the version directory for this note
            note_version_dir = self._get_note_version_dir(note_id)
            version_info_path = self._get_version_info_path(note_id)
            ...
            # Get version info to find all version files
            with open(version_info_path, 'r', encoding='utf-8') as f:
                version_info = json.load(f)
            ...
            # Try to remove the note version directory if it's empty
            try:
                os.rmdir(note_version_dir)
