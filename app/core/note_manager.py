...
from app.utils.html_exporter import convert_markdown_to_html, create_html_index, export_note_to_html
from app.utils.pdf_exporter import convert_markdown_to_pdf, export_note_to_pdf
...
class NoteManager:
    ...
    def export_note_to_pdf(self, title: str, output_path: Optional[str] = None,
                           category: Optional[str] = None, output_dir: Optional[str] = None,
                           custom_css: Optional[str] = None, include_metadata: bool = True) -> Tuple[bool, str]:
        """
        Export a note to PDF format.

        Args:
            title: The title of the note to export.
            output_path: Optional specific path for the PDF file. 
                        If not provided, derives from the title.
            category: Optional category to help find the note.
            output_dir: Optional specific directory to look for the note.
            custom_css: Optional custom CSS for styling the PDF.
            include_metadata: Whether to include note metadata in the PDF.
        ...
        try:
            ...
            # If no output path specified, create one based on the note title
            if not output_path:
                ...
                output_path = os.path.join(exports_dir, pdf_filename)
            ...
            # Add created_at and updated_at to metadata if they exist as strings
            ...
            if success:
                return True, f"Note exported successfully to {output_path}"
            else:
                return False, "Failed to export note to PDF. Check logs for details."
                
        except Exception as e:
            return False, f"Error exporting note to PDF: {str(e)}"
            
    def export_notes_to_pdf(self, notes: List[str], output_dir: str,
                            category: Optional[str] = None, source_dir: Optional[str] = None,
                            custom_css: Optional[str] = None, include_metadata: bool = True) -> Dict[str, str]:
        """
        Export multiple notes to PDF format.
        
        Args:
            notes: List of note titles to export.
            output_dir: Directory to save the exported PDFs.
            category: Optional category to help find the notes.
            source_dir: Optional specific directory to look for the notes.
            custom_css: Optional custom CSS for styling the PDFs.
            include_metadata: Whether to include note metadata in the PDFs.
            
        Returns:
            Dictionary with note titles as keys and export status messages as values.
        """
        # Ensure output directory exists
        ...
        for title in notes:
            ...
            # Export the note
            success, message = self.export_note_to_pdf(
                title=title,
                output_path=output_path,
                category=category,
    ...
    def export_all_notes_to_pdf(self, output_dir: str, category: Optional[str] = None,
                               source_dir: Optional[str] = None, custom_css: Optional[str] = None,
                               include_metadata: bool = True) -> Tuple[int, int, List[str]]:
        """
        Export all notes to PDF format.
        ...
        Returns:
            Tuple of (number of successful exports, total number of notes, list of failed notes)
        """
        # Ensure output directory exists
        output_dir = os.path.expanduser(output_dir)
        os.makedirs(output_dir, exist_ok=True)
        ...
        # Get all note files
        note_files = list_note_files(source_dir or self.notes_dir, category)
        ...
        for note_file in note_files:
            try:
                # Read the note to get metadata
                metadata, _ = read_note_file(note_file)
                title = metadata.get('title', os.path.splitext(os.path.basename(note_file))[0])
                ...
                # Add the filename if we couldn't even get the title
                failed_notes.append(os.path.basename(note_file))
                
        return successful, len(note_files), failed_notes
    ...
    # HTML export methods
    def export_note_to_html(self, title: str, output_path: Optional[str] = None,
                           category: Optional[str] = None, output_dir: Optional[str] = None,
                           custom_css: Optional[str] = None, include_metadata: bool = True) -> Tuple[bool, str]:
        """
        ...
        Args:
            title: The title of the note to export.
            output_path: Optional specific path for the HTML file. 
                        If not provided, derives from the title.
            category: Optional category to help find the note.
            output_dir: Optional specific directory to look for the note.
            custom_css: Optional custom CSS for styling the HTML.
            include_metadata: Whether to include note metadata in the HTML.
        ...
        try:
            ...
            if not output_path:
                ...
                html_filename = f"{slugify(title)}.html"
                ...
                # Add exports directory
                exports_dir = os.path.join(base_dir, "exports", "html")
                os.makedirs(exports_dir, exist_ok=True)
                
                # Full path for the HTML
                output_path = os.path.join(exports_dir, html_filename)
            ...
            self._format_dates_for_display(metadata)

            # Export to HTML
            success = export_note_to_html(
                note_content=content,
                title=metadata.get('title', title),
                output_path=output_path,
                metadata=metadata,
            ...
            if success:
                ...
                return False, "Failed to export note to HTML. Check logs for details."
                
        except Exception as e:
            return False, f"Error exporting note to HTML: {str(e)}"

    def export_notes_to_html(self, notes: List[str], output_dir: str, create_index: bool = True,
                            category: Optional[str] = None, source_dir: Optional[str] = None,
                            custom_css: Optional[str] = None, include_metadata: bool = True) -> Dict[str, str]:
        """
        ...
        Args:
            notes: List of note titles to export.
            output_dir: Directory to save the exported HTMLs.
            create_index: Whether to create an index.html file.
            category: Optional category to help find the notes.
            source_dir: Optional specific directory to look for the notes.
            custom_css: Optional custom CSS for styling the HTMLs.
        ...
        for title in notes:
            ...
            # Find the note to get metadata
            note_path = self.find_note_path(title, category, source_dir)
            if note_path:
                metadata, _ = read_note_file(note_path)
                # Format dates for display
                self._format_dates_for_display(metadata)
                
                # Add note info for the index
                note_info = {
                    'title': title,
            ...
            # Export the note
            success, message = self.export_note_to_html(
                title=title,
                output_path=output_path,
                category=category,
                output_dir=source_dir,
        ...
        # Create an index file if requested
        if create_index and note_info_list:
            index_path = os.path.join(output_dir, "index.html")
            create_html_index(
                notes_info=note_info_list,
                output_path=index_path,
                group_by_category=True,
                custom_css=custom_css
            )
            results['index'] = f"Index created at {index_path}"
            
        return results
                
    def export_all_notes_to_html(self, output_dir: str, create_index: bool = True,
                               category: Optional[str] = None, source_dir: Optional[str] = None,
                               custom_css: Optional[str] = None, 
                               include_metadata: bool = True) -> Tuple[int, int, List[str], Optional[str]]:
        """
        Export all notes to HTML format.
        ...
        return successful, len(note_files), failed_notes, index_path
